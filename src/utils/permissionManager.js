...
export default class PermissionManager {
  ...
  static requestPermission(perm, resolve, permissionBlockedDialog, isBluetooth) {
   Permissions.request(perm)
     .then((result) => {
       if (
         ...
         permissionBlockedDialog().then((isNeeded) => {
           if (isNeeded) {
             perm===Permissions.PERMISSIONS.IOS.LOCATION_WHEN_IN_USE? Linking.openURL('App-Prefs:Privacy&path=LOCATION'): Linking.openSettings();
             resolve(PERMISSION_RESULT.DENIED);
           }
         });
       }
  ...
  static handlePermissions(perm,permissionRationaleDialog, permissionBlockedDialog, resolve, isBluetooth = false) {
        Permissions.check(perm)
          .then((result) => {
            if (result === PERMISSION_RESULT.DENIED) {
              permissionRationaleDialog().then((isNeeded) => {
                if (isNeeded) {
                 this.requestPermission(perm, resolve, permissionBlockedDialog, isBluetooth);
                }
              });
            } else if (
              result === Permissions.RESULTS.GRANTED ||
